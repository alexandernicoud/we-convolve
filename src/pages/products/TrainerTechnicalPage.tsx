...
export default function TrainerTechnicalPage() {
  ...
  const handleStartTraining = async () => {
    ...
    try {
      const response = await fetch('/trainer/runs', {
        method: 'POST',
  ...
  const handleStartAnalysis = async () => {
    ...
    try {
      const response = await fetch(`/trainer/runs/${runId}/analysis`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({}),
      });
  ...
  const startPolling = (runId: string) => {
    ...
    const interval = setInterval(async () => {
      try {
        const response = await fetch(`/trainer/runs/${runId}`);
  ...
  const startAnalysisPolling = (runId: string, analysisId: string) => {
    const interval = setInterval(async () => {
      try {
        const response = await fetch(`/trainer/runs/${runId}/analysis/${analysisId}`);
  ...
  const handleDownloadModel = () => {
    if (!runId) return;
    window.open(`/trainer/runs/${runId}/download/model`, '_blank');
  };

  const handleDownloadAnalysis = () => {
    if (!runId || !analysisId) return;
    window.open(`/trainer/runs/${runId}/analysis/${analysisId}/download`, '_blank');
  };
      ...
      <div className="grid grid-cols-1 gap-4">
        <div>
          <label className="block text-sm text-[#F5F7FF]/62 mb-2">
            Training Folder <span className="text-[#22D3EE]">*</span>
          </label>
          <input
            type="text"
          ...
          />
          <p className="text-xs text-[#F5F7FF]/50 mt-1">Path to folder containing labeled PNG images</p>
        </div>
